...
function validateTweetData(content: string): { isValid: boolean; error?: string } {
    ...
    // Check for data consistency in comparisons (→)
    const comparisons = content.match(/\$?\d+\.?\d*[BMK]?\s*→\s*\$?\d+\.?\d*[BMK]?/g) || [];
    for (const comparison of comparisons) {
        const [before, after] = comparison.split('→').map(s => s.trim());
...
function generateDegenComment(content: string, learningClient?: TwitterLearningClient): string | null {
    ...
    if (Math.random() < 0.3) {
        ...
        const degenPhrases = {
            bullish: [
                "ngmi if you're not in this 🚀",
                "ser, this is the gwei 🌊",
                "looks undervalued tbh 👀",
                "wen moon? probably soon 🌙",
                "based metrics 🔥",
                "bullish af on these numbers 📈",
                "wagmi frens 🤝",
                "absolutely massive if true 🤯",
                "chad moves only 💪",
                "few understand this 🧠",
                "ser, you might be early 🎯",
                "this is the alpha we need 📊",
                "degen approved ✅",
                "imagine not being long here 🎯",
                "anon, load your bags 💰",
                "literally free alpha ser 🎁",
                "top signal? nah, just getting started 📈",
                "ser, this is financial advice 🤝",
                "aping in rn 🦍",
                "looks like a proper send 🚀"
            ],
            bearish: [
                "still bullish tbh 💎🙌",
                "buying the dip ser 🎯",
                "weak hands getting rekt 👋",
                "shaking out the paper hands 📄",
                "accumulation zone detected 👀",
                "time to stack sats 🏦",
                "fear is temporary, ngmi is forever 💪",
                "imagine selling here 🤡",
                "ser, this is a gift 🎁",
                "bottom signal? probably 📊"
            ],
            neutral: [
                "this is the way 🫡",
                "big if true 👀",
